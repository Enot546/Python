...
from form_allure.form_pages.main_page import MainPage
from form_allure.form_pages.submited_page import SubmitedPage
...
class TestForm:
    ...
    def test_empty_zip_code(
        ...
        with allure.step('Проверить, что у пустого поля индекс CSS-свойства background-color и border-color соответствуют danger'):  # noqa: E501
            if not self.is_colors_match(submited_page.get_zip_code_bg_color(),
                                        error_bg_color):
                errors.append('ERROR in danger background-color css property for zip-code field')  # noqa: E501
            if not self.is_colors_match(submited_page.get_zip_code_brd_color(),
                                        error_brd_color):
                errors.append('ERROR in danger border-color css property for zip-code field')  # noqa: E501

        with allure.step('Проверить, что у заполненного поля имя CSS-свойства background-color и border-color соответствуют success'):  # noqa: E501
            if not self.is_colors_match(submited_page.get_first_name_bg_color(), success_bg_color):
                errors.append('ERROR in success background-color css property for first-name field')  # noqa: E501
            if not self.is_colors_match(submited_page.get_first_name_brd_color(), success_brd_color):
                errors.append('ERROR in success border-color css property for first-name field')  # noqa: E501

        with allure.step('Проверить, что у заполненного поля фамилия CSS-свойства background-color и border-color соответствуют success'):  # noqa: E501
